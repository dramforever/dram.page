...
当然，我们也有不太顺利的时候。中间有一段时间，我们发现转发部分的逻辑有性能问题，十分困惑，改的时候也很头疼，越改性能越差，改着改着干脆就不通了。

回到最开始至少能通的版本，想到之前加入了一个状态机的原因是 AXI Stream 的流水线 `TREADY` 信号的组合逻辑的最坏路径过长导致出现时序问题，而事实上连续的单周期流水线级要停大家一起停，也就是说 `TREADY` 本身并不需要逻辑。但是实验框架里给的示例中的 `S_AXIS_TREADY` 是 `M_AXIS_TREADY || ! S_AXIS_TVALID`，也就是说，`S_AXIS_TVALID` 为低时，我们要把 `S_AXIS_TREADY` 拉高，这是完全没有意义的。最开始注意到这个问题的时候，我没有太在意，没想到它竟然成为了坑了我们好几天的一个问题。

把代码回滚到之前有时序问题的版本，去掉 `TREADY` 依赖 `TVALID` 的逻辑之后，性能立刻就正常了。
...
在答辩的最后，我插播了一段私货。事实上，这段私货的内容是我前一天的晚上才准备好的，当时事实上紧急叫了几个朋友帮忙看看有没有什么写的不太妙的地方，做了一些细微修改。这段在演讲的时候，内容我自己有所发挥，但是大致的文字内容是前一天写好的，在此：
